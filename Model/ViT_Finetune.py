...
mod = torch.load(pretrained_weights_path)
count_module = 0
for i in model.modules():
    if(isinstance(i, nn.modules.conv.Conv2d) and count_module==0):
        count_module = 1
        patch_embed = list(i.named_parameters())
        for j in patch_embed:
            j[1].data = torch.nn.Parameter(mod['patch_embed.proj.'+ j[0]])

    if(isinstance(i, nn.modules.container.ModuleList)):
        block = list(i.named_parameters())
        for j in block:
            # print(j[0])
            j[1].data = torch.nn.Parameter(mod['blocks.'+ j[0]])

    if(isinstance(i, Cls_token)):
        class_token = list(i.named_parameters())
        for j in class_token:
            j[1].data = torch.nn.Parameter(mod[j[0]])
